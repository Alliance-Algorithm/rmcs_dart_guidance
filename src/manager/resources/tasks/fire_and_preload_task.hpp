...
public:
    FireAndPreloadTask(
        ...
        if (runtime_state.fire_count > 0) {
            ...
            static constexpr uint64_t kLimit_fill_ticks[] = {25, 25, 25};
            then(
                std::make_shared<FillingLimitServoAction>(
                    ...
                    settings.limiting_fill_ticks
                        * kLimit_fill_ticks[runtime_state.fire_count - 1] // 预装填持续帧数
                    ));

            then(
                std::make_shared<FillInitialFlagAction>(
                    "filling_initial",                                    // 动作名称
                    output.lifting_init_flag                              // 重置最高点位置
                    ));
        }
    }
};
