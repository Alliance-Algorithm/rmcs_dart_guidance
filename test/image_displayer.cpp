...
private:
    void initialize() {
        ...
        if (display_raw_) {
            raw_image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
                raw_image_topic_, 10, [this](const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        ...
        if (display_processed_) {
            ...
            RCLCPP_INFO(
                logger_, "Subscribed to processed image topic: %s", processed_image_topic_.c_str());
        ...
        if (display_available_) {
            ...
            display_thread_ = std::thread(&ImageDisplayer::display_loop, this);
    ...
    void display_loop() {
        ...
        if (display_raw_) {
            ...
            cv::resizeWindow(
                "Raw Image", static_cast<int>(640 * window_scale_),
        ...
        if (display_processed_) {
            ...
            cv::resizeWindow(
                "Processed Image", static_cast<int>(640 * window_scale_),
                static_cast<int>(480 * window_scale_));
        }
        ...
        while (display_running_ && rclcpp::ok()) {
            try {
                    ...
                    cv::Mat display_raw;
                    {
                        ...
                        if (!raw_image_.empty()) {
                            ...
                            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                               now - raw_image_timestamp_)
                                               .count();

                            if (elapsed > 500) {
                                cv::putText(
                                    display_raw, "STALE", cv::Point(10, 30),
                    ...
                    cv::Mat display_processed;
                    {
                        ...
                        if (!processed_image_.empty()) {
                            ...
                            if (display_processed.channels() == 1) {
                                cv::cvtColor(
                                    display_processed, display_processed, cv::COLOR_GRAY2BGR);
                            }
                            ...
                            // Add target information
                            {
                                ...
                                if (target_tracking_) {
                                    cv::circle(
                                        display_processed, target_position_, 20,
                                        cv::Scalar(0, 255, 255), 2);
                                    cv::putText(
                                        display_processed, "TRACKING", cv::Point(10, 30),
                            ...
                            auto now = std::chrono::steady_clock::now();
                            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                               now - processed_image_timestamp_)
                                               .count();

                            if (elapsed > 500) {
                                cv::putText(
                                    display_processed, "STALE", cv::Point(500, 30),
                                    cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
                            }
                        }
                    }
                ...
                if (frame_counter % 60 == 0) {
                    auto now = std::chrono::steady_clock::now();
                    auto elapsed =
                        std::chrono::duration_cast<std::chrono::milliseconds>(now - last_fps_time)
                            .count();
...
PLUGINLIB_EXPORT_CLASS(rmcs_dart_guidance::ImageDisplayer, rmcs_executor::Component)